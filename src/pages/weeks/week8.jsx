...
function Week8() {
  ...
  const images = ["/images/FirstPuasa.png", "/images/FollowUp.png"];
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-6" style={{ position: "relative", zIndex: 10 }}>
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-blue-700 dark:text-blue-700 mb-2">Week 8</h1>
          <p className={`text-lg mb-8 ${darkMode ? "text-gray-400" : "text-gray-600"}`}>01 March 2026 - 05 March 2026</p>
          ...
          <div className="flex flex-col items-center mb-8 w-full max-w-lg mx-auto">
            ...
            >
              <div className="flex h-full transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
                {images.map((src, idx) => (
                  <img key={idx} src={src} alt={`Week 8 Image ${idx + 1}`} className="w-full h-full flex-shrink-0 object-contain bg-black" />
                ))}
              </div>
          ...
          <div className={`${darkMode ? "text-gray-200" : "text-gray-700"}`}>
            <p className="text-lg leading-relaxed">Week 8 involved managing campaign tasks and preparing materials for follow-up activities.</p>
          </div>
        </div>
      </div>
    </section>
